...
def test_basic(user3_old_key):
	...
	for existing in ('user1', 'user2', 'user3', 'user5', 'userTest'):
		...
		if existing == 'user1':
			...
			assert ['ssh-ed25519 AAAAmviuewjuivjrvenuvlejnreiuvwejievwojviovewiofoobar\n', 'ssh-ed25519 AAAACvSDMI62OVMImv2eMVMS5DIOV346EMVWIEO something\n'].sort() == as_list_2.sort()
		elif existing == 'user3':
...
def test_ignored(user3_old_key):
	...
	for existing in ('user3',):
		...
		assert ['ssh-ed25519 AAAAAremovemeremovemeREMOVEMEremoveme123 oldololdold\n'] == as_list
