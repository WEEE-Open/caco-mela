...
def parse_args(tests_env: Optional[str] = None) -> dict[str, Union[str, bool]]:
    parser = argparse.ArgumentParser(
        description="Provision SSH keys from a LDAP server, without syncing UIDs.",
        prog="caco-mela",
    )
    parser.add_argument("--version", action="version", version="%(prog)s 1.0.0")
    parser.add_argument(
        "-l", "--ldap", dest="LDAP_BIND_SERVER", type=str, help="LDAP server address"
    )
    parser.add_argument(
        "-D", "--binddn", dest="LDAP_BIND_DN", type=str, help="LDAP bind DN"
    )
    parser.add_argument(
        "-w", "--bindpw", dest="LDAP_BIND_PASSWORD", type=str, help="LDAP bind password"
    )
    parser.add_argument(
        "-t",
        "--starttls",
        action="store_true",
        dest="LDAP_STARTTLS",
        help="Use LDAP_STARTTLS",
    )
    parser.add_argument(
        "-s", "--search", dest="LDAP_SEARCH_BASE", type=str, help="LDAP search base"
    )
    parser.add_argument(
        "-f", "--filter", dest="LDAP_FILTER", type=str, help="LDAP filter"
    )
    parser.add_argument(
        "--key",
        dest="LDAP_SEARCH_SSH_KEY_ATTR",
        type=str,
        help="Attribute containing the SSH public key",
    )
    parser.add_argument(
        "--uid",
        dest="LDAP_SEARCH_SSH_UID_ATTR",
        type=str,
        help="Attribute containing the username",
    )
    parser.add_argument(
        "-a",
        "--authorized",
        dest="SSH_AUTHORIZED_KEYS_FILES",
        type=str,
        help="Value of sshd option AuthorizedKeysFile",
    )
    parser.add_argument(
        "--user-owns-file",
        dest="SSH_USER_OWNS_FILE",
        action="store_true",
        help="Users are set to owners of their authorized_keys file, if the file is created",
    )
    parser.add_argument("-v", "--verbose", action="store_true", help="Verbose output")
    parser.add_argument(
        "IGNORED_ACCOUNTS", nargs="*", type=str, help="Accounts to ignore"
    )
    ...
    if len(config["IGNORED_ACCOUNTS"]) == 0:
        config["IGNORED_ACCOUNTS"] = set(
            os.environ.get("IGNORED_ACCOUNTS", default="").split(",")
        )
    else:
...
def get_data_from_server(config):
    ...
    try:
        ldap_result_id = conn.search(
            ...
            ldap.SCOPE_SUBTREE,
            config["LDAP_FILTER"],
            [config["LDAP_SEARCH_SSH_KEY_ATTR"], config["LDAP_SEARCH_SSH_UID_ATTR"]],
        )
        while 1:
            ...
            if result_type == ldap.RES_SEARCH_ENTRY:
                ...
                if config["LDAP_SEARCH_SSH_UID_ATTR"] in result_data[0][1]:
                    if config["LDAP_SEARCH_SSH_KEY_ATTR"] in result_data[0][1]:
                        add[
                            result_data[0][1][config["LDAP_SEARCH_SSH_UID_ATTR"]][
                                0
                            ].decode("ascii")
                        ] = [
                            x.decode("ascii")
                            for x in result_data[0][1][
                                config["LDAP_SEARCH_SSH_KEY_ATTR"]
                            ]
                        ]
                    else:
                        add[
                        ...
                        if config["verbose"]:
                            print(
                                f"No attribute {config['LDAP_SEARCH_SSH_KEY_ATTR']} for user {result_data[0][0]}, SSH keys will be removed"
                            )
                else:
                    if config["verbose"]:
                        print(
                            f"No attribute {config['LDAP_SEARCH_SSH_UID_ATTR']} for user {result_data[0][0]}, ignoring"
                        )
            else:
                break
    except ldap.LDAPError as e:
...
def ssh_authorized_keys_file(config, user: pwd.struct_passwd, create: bool = True):
    ...
    if create and not os.path.exists(path):
        ...
        if config["SSH_USER_OWNS_FILE"]:
            if config["verbose"]:
                print(
                    f"Setting owner to {user.pw_uid}:{user.pw_gid} and mode to 600 for file {path}"
                )
        ...
        if uid_min <= user.pw_uid <= uid_max:
            ...
            if user.pw_name in results:
                ...
                if update_file(ssh_file, text):
                    print(
                        f"Updated user {user.pw_name} with {str(len(results[user.pw_name]))} SSH keys"
                    )
                elif config["verbose"]:
                    print(
                        f"No change for user {user.pw_name} with {str(len(results[user.pw_name]))} SSH keys"
                    )
            else:
                ...
                if os.path.exists(ssh_file):
                    if config["verbose"]:
                        print(
                            f"User {user.pw_name} not found in LDAP server, removing keys"
                        )
                    text = generate_text([])
                    if update_file(ssh_file, text):
